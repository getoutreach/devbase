...
#   log_github_rate_limit "$token" "setup_end"
...
#   GITHUB_RATE_LIMIT|<phase>|core:<used>/<limit>|remaining:<N>|resets_in:<seconds>s|pct_used:<N>%|repo:<CIRCLE_PROJECT_REPONAME>|job:<CIRCLE_JOB>
#
...
log_github_rate_limit() {
  ...
  echo "GITHUB_RATE_LIMIT|${phase}|core:${used}/${limit}|remaining:${remaining}|resets_in:${resets_in}s|pct_used:${pct_used}%|repo:${repo}|job:${job}"
