...
prettier_linter() {
  ...
  find_files_with_extensions "${extensions[@]}" | xargs -n40 yarn run prettier -l --loglevel log
  return $?
}

prettier_formatter() {
  yarn_install_if_needed >/dev/null
  find_files_with_extensions "${extensions[@]}" | xargs -n40 yarn run prettier --write --loglevel warn
}
